      ...
      {total>0?(
        <div style={{display:"flex",flexWrap:"wrap",gap:5}}>
          {pillarAnalysis.map(p=>{
            ...
            return(
              <div key={p.name} style={{display:"flex",alignItems:"center",gap:4,borderRadius:7,padding:"4px 9px",background:ok?`${p.bg}33`:needMore?`${CORAL}18`:`${GREEN}18`,border:`1px solid ${ok?p.bg+"55":needMore?`${CORAL}44`:`${GREEN}44`}`}}>
                <span style={{fontFamily:IN,fontSize:10,fontWeight:700,color:ok?p.color:needMore?CORAL:"#A8D672"}}>{p.name}</span>
                <span style={{fontFamily:IN,fontSize:10,fontWeight:600,color:TX3}}>{p.actual}%</span>
                {needMore&&<span style={{fontFamily:IN,fontSize:9,fontWeight:700,color:CORAL}}>↑ need {Math.abs(p.diff)} more</span>}
                {!ok&&!needMore&&<span style={{fontFamily:IN,fontSize:9,fontWeight:700,color:"#A8D672"}}>↓ reduce by {p.diff}</span>}
              </div>
            );
          })}
        </div>
